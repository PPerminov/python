...
def matrix_transpose(matrix):
    ...
    # new cols. All new cols must be = rows. We will need to count
    rows = size[0]
...
def matrix_multiply(matrix1, matrix2):
    ...
    if type(matrix2) == int:
        for row in range(0, matrix_size(matrix1)[0], 1):
            new_matrix.append([])
            for col in range(0, matrix_size(matrix1)[1], 1):
                new_matrix.append(matrix_get_position(
    ...
    for row in range(0, new_matrix_size[0], 1):
        ...
        for col in range(0, new_matrix_size[1], 1):
            ...
            for position in range(0, new_matrix_size[0], 1):
                value += matrix_get_position(matrix1, row, position) * \
                    matrix_get_position(matrix2, col, position)
