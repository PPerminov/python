...
def pi1(rounds):
    ...
    k = 0
    while k <= rounds:
        answer += (factorial(6 * k)*(13591409 + (545140134 * k)))/(factorial(3 * k) * pow(factorial(k),3)*pow(-640320 , 3 * k))
        k += 1
...
def pi2(rounds):
    def worker(k,accum):
        if k > rounds:
            return accum
        k8 = 8 * k
        data = Fraction(1,pow(16,k))*(Fraction(4 , k8 + 1)-Fraction(2,k8 + 4)-Fraction(1 , k8 + 5)-Fraction(1/k8 + 6))
...
def pi4(rounds):
    def worker(k,accum):
        if k > rounds:
            return accum
        k8 = 8 * k
        data = (1 / pow(16,k)) * ((4 / (k8 + 1)) - (2 / (k8 + 4)) - (1 / (k8 + 5)) - (1 / (k8 + 6)))
        return worker(k+1,accum + data)
    return worker(0,0)
